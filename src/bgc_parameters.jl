...
@latexSymbol @description @flattenable @prior @printunits @units @default_kw struct Para{U} <: AbstractPara{U}
    umax::U |  1e-6 / spd | u"mol/m^3/s" | u"mmol/m^3/d" | LN(1e-6 / spd, 1e-6 / spd) | true  | "Maximum uptake rate (Michaelis-Menten)"      | "\\mathbf{u}_\\mathrm{max}"
    ku  ::U |        1e-4 | u"mol/m^3"   | u"mmol/m^3"   | LN(1e-4, 0.5e-4)           | true  | "Half-saturation constant (Michaelis-Menten)" | "k_\\mathbf{u}"
